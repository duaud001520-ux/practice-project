...
def main() -> None:
    ...
    for wav_path, ref, hyp in results:
        sample_wer = jiwer.wer(ref, hyp) if ref.strip() else 0.0
        scored.append((sample_wer, wav_path, ref, hyp))
    scored.sort(key=lambda x: x[0], reverse=True)

    print(f"=== WER 최악 {args.top_k}개 샘플 (n={len(subset)}) ===")
    for sample_wer, wav_path, ref, hyp in scored[: args.top_k]:
        print(f"\n[{wav_path.name}] WER={sample_wer * 100:.1f}%")
        print(f"  정답: {ref}")
        print(f"  예측: {hyp}")
